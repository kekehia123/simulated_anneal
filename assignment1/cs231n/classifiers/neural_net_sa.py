...
class TwoLayerNet_sa(object):
  ...
  def __init__(self, input_size, hidden_size, output_size, std=1e-4, opt_method = 'grad'):
    """
  ...
  def calc_grads(self, net_loss, X, y=None, reg=0.0):
    ...
    if opt_method == 'sa':
        #coefs = np.concatenate((np.reshape(W1, -1), np.reshape(b1, -1), 
        #                np.reshape(W2, -1), np.reshape(b2, -1)), axis = 0)
        x = self.sa_opt(self.loss, self.params, 1000, 0.01, 2, 200, 1e-3, X, y, reg)
        params_name = list(x.keys())
        for param in params_name:
            grads[param] = x[param] - self.params[param]
    return grads

  def sa_opt(func, init_guess, T_max, T_min, step_size, max_iter, eps, X, y, reg):
    ...
    for i in range(max_iter):
        ...
        current = func(x, X, y, reg)
        x_new = {}
        for param in params_name:
            x_new[param] = x[param] + step_size * (np.random.uniform(0, 1, len(x[param])) - 0.5) * 2
        proposal = func(x_new, X, y, reg)
